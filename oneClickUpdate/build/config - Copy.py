...
LimitToVerifyUpdateID = {1 : int(const.CUSTOM_JOBS_LIMIT)+int(const.QH_RELEASE_DIFF_LIMIT),2:int(const.CUSTOM_JOBS_LIMIT)+int(const.E2_RELEASE_DIFF_LIMIT)}
...
class MyDB(object):
    ...
    def __init__(self):
        ...
        user = request.user
        group = User.objects.filter(groups__name='ViewAndWrite')
        if user in group:
