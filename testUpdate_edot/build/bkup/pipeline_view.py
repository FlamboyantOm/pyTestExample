        ...
        if str(step) == str(1):
            ...
            try:
                ...
                if data_row[1] != 3:
                    ...
                    return HttpResponse(json.dumps(data), content_type='application/json')
                sql = "UPDATE jobinfo  SET `JobState`='4' WHERE  JobId = '"+str(jobId)+"'  ;"
                flib.db_insert(sql)
                sql = "INSERT INTO jobstatedetails (`JobID`, `JobState`,`PercentDone`) VALUES ('"+str(jobId)+"', '4', '100');"
                flib.db_insert(sql)
            ...
            try:
                ...
                if data_row[1] != 5 :
                    ...
                    return HttpResponse(json.dumps(data), content_type='application/json')
                sql = "UPDATE jobinfo  SET `JobState`='6' WHERE  JobId = '"+str(jobId)+"'  ;"
                flib.db_query(sql)
                ...
                if request.method == 'POST':
                    ...
                    if reqType != 0 :
                        ...
                        if request.POST['VDB'] == '':
                            vdb = datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d %H:%M:%S')
                        else:
                            vdb = request.POST['VDB']
                        note = request.POST['Note']
                    sql = "INSERT INTO releaseinfo (`JobID`, `Notes`, `VDB`, `UserName`) VALUES ('"+str(jobId)+"','"+note+"','"+str(vdb)+"', '"+str(userName)+"');"
                    #backup_e2
                    try:
                    ...
                    flib.db_insert(sql)
                    sql = "UPDATE qhpackagedetails  SET `PackageState`='6' WHERE  JobID = '" + str(jobId) + "' ;"
                    flib.db_insert(sql)
            ...
            try:
                ...
                sql = "INSERT INTO jobstatedetails (`JobID`, `JobState`,`PercentDone` ) VALUES ('" + str(insertId) + "', '1','100');"
                flib.db_insert(sql)
                sql = "INSERT INTO jobproperty (`JobID`, `Name`,`Value`) VALUES ('" + str(insertId) + "', 'REVERTED_FROM', '"+str(revertJobId)+"');"
                flib.db_insert(sql)
                paramArr.update({'Job_ID': str(insertId)})
        ...
        if str(step) == str(1):
            try:
                ...
                for url in Package_ListArray.split(","):
                    if cnt == 0:
                        cnt = 1
                        Package_ListArrayWhere += ' PackageID = "'+str(url)+'" '
                    else :
                        Package_ListArrayWhere += ' or PackageID = "'+str(url)+'" '
                sql = "SELECT * FROM qhpackagedetails where "+str(Package_ListArrayWhere)+" and PackageState = 6  order by JobID desc"
                ...
                if len(data_row) != 0:
                    ...
                    sql = "INSERT INTO jobstatedetails (`JobID`, `JobState`,`PercentDone` ) VALUES ('"+str(insertId)+"', '1','100');"
                    flib.db_insert(sql)
                    #for package_id in Package_ListArray.split(","):
                        #sql = "INSERT INTO qhpackagedetails (`JobID`, `PackageID`,`PackageState` ) VALUES ('" + str(insertId) + "', '" + package_id + "','2');"
                        #flib.db_insert(sql)
            ...
            try:
                ...
                if apiResult['error_code'] == 0 and apiResult['status'] == 1:
                    data =  {'status': 2, 'msg': 'Verification Failed.'}
                    return HttpResponse(json.dumps(data), content_type='application/json')
                elif apiResult['error_code'] == 0 and apiResult['status'] == 0:
                    pass
                else:
                    data =  {'status': 2, 'msg': 'Error in Verificatino script'}
                    return HttpResponse(json.dumps(data), content_type='application/json')
                ...
                if request.method == 'POST':
                    ...
                    try:
                        ...
                        print('Error (backup_qh jenkins job) ',e)
                    flib.db_insert(sql)
                    sql = "UPDATE jobinfo  SET `JobState`='6' WHERE  JobId = '" + str(jobId) + "' and EngineType = 1;"
                    flib.db_insert(sql)
                data = {'status':1,'msg':'Job #'+str(jobId)+' mark released successfully !'}  # create json array to response json
                return HttpResponse(json.dumps(data), content_type='application/json')
            except Exception as e:
...
def failJobProcess(engine,reason,user):
    try:
        ...
        jobId = data_row[0]
        sql = "UPDATE jobinfo SET `JobState`= '7' where EngineType = '"+str(engine)+"' and  JobId = '"+str(jobId)+"'"
        flib.db_insert(sql)
        sql = "INSERT INTO jobstatedetails (`JobID`, `JobState`) VALUES ('"+str(jobId)+"', '7');"
        flib.db_insert(sql)
        sql = "INSERT INTO failedjobinfo (`JobID`, `FailedReason`, `Notes`, `UserName`) VALUES ('"+str(jobId)+"','3','"+str(reason)+"','"+str(user)+"');"
        flib.db_insert(sql)
...
def build_engine_mgt_ajax(request,type,key=0,action=0):
    if not type in config.EngineTypeArr:
        return HttpResponse('Invalid request')
    if str(key) != str(0):
        sql = 'UPDATE releaseinfo SET IsBuildEngine='+str(action)+' WHERE  JobID='+str(key)+' LIMIT 1;'
        flib.db_insert(sql)
    ...
    db_obj_total_row = flib.db_query(sql,1)
    column = {0:'JobID',1:'CreatedDate',2:'UserName',3:'Notes',4:'VDB',5:'IsBuildEngine'}
    order_column = column[int(request.GET['order[0][column]'])]
    order_column_dir = request.GET['order[0][dir]']
    if(search_value != ''):
...
def build_engine_mgt(request,type):
    ...
    getIsBuildEngineDetails,getUpdateID,getUpdateIDName = '','',''
    if request.GET and 'UpdateID' in request.GET:
        getUpdateID = request.GET['UpdateID']
        getIsBuildEngineDetails = getisbuildengineDetails(str(config.EngineTypeArr[type]),getUpdateID)
        getUpdateIDName = str(config.UpdateTypeOnly[int(getUpdateID)]) + ' (' + getUpdateID + ')'
    UpdateIdList = getUpdateId(str(config.EngineTypeArr[type]), getUpdateID)
    return render(request, 'build/build_engine_mgt.html', {'type':type,'UpdateIdList':UpdateIdList,'getIsBuildEngineDetails':getIsBuildEngineDetails,'getUpdateIDName':getUpdateIDName})

def getisbuildengineDetails(type,UpdateId):
    sql = 'SELECT jobinfo.JobID, jobinfo.DateCreated AS CreatedDate, jobinfo.UserName AS UserName, releaseinfo.Notes AS Notes, releaseinfo.VDB AS VDB, releaseinfo.IsBuildEngine AS IsBuildEngine,buildinfo.RemovedBy  ,enginemaster.UpdateID FROM jobinfo LEFT JOIN releaseinfo ON releaseinfo.JobID = jobinfo.JobID LEFT JOIN buildinfo ON buildinfo.JobID = jobinfo.JobID LEFT JOIN enginemaster ON enginemaster.JobID = jobinfo.JobID  AND enginemaster.UpdateID = '+str(UpdateId)+' WHERE jobinfo.JobID = releaseinfo.JobID AND jobinfo.EngineType = '+str(type)+' ORDER BY releaseinfo.JobID DESC'
    data_row = flib.db_query(sql)
    detailList = []
    if data_row is not None:
        for row in data_row :
            detailList.append({'JobID':str(row[0]),'CreatedDate':str(row[1]),'UserName':str(row[2]),'Notes':str(row[3]),'VDB':str(row[4]),'IsBuildEngine':str(row[5]),})
    return detailList

def getUpdateId(type,getUpdateID =''):
    updateIdList = config.UpdateID[int(type)]
    updateIdDropdown = ''
    updateIdDropdown += "<div class='text-center col-md-3'>"
    ...
    for ui in updateIdList:
        print(str(getUpdateID))
        if str(getUpdateID) == str(ui):
            selected = 'selected=selected'
        else:
            selected = ''
        updateIdDropdown += '<option '+selected+' value="' + str(ui) + '" >' + str(config.UpdateTypeOnly[int(ui)]) + '</option>'
...
def build_id_details(request,pid):
    sql = "SELECT * FROM %s where JobId = %s " % ("jobinfo", pid)
    jobInfo = flib.db_query(sql, 1)
    flib.setMetaInformation(request, 'Job Details : #' + pid, OrderedDict([('Job History : '+config.EngineType[jobInfo[3]], '/history/' + config.EngineType[jobInfo[3]].lower() + '/'), ('Job Details : #' + pid, '')]))
    jobId,JobStatus,flag = jobInfo[0],jobInfo[1],jobInfo[4]
    content = '<div id="build-details">'
    content += getBuildDetailsBasic(jobInfo)
    content += '</div>'
    ...
    if updateIDArr:
        content += getSoloInfo(jobId,flag,JobStatus,updateIDArr,config.EngineType[jobInfo[3]])
        content += getDiffInfo(jobId,flag,JobStatus,updateIDArr,config.EngineType[jobInfo[3]])
...
def previous_builds(request,type,build_type=1):
    offset,perPage = 1,10
    content = '<div class="previous-builds">' + flib.previousBuildHtml(type,build_type,offset, 15,request) + '</div>'
    data = {'data': content, 'show':'completed'}
    return HttpResponse(json.dumps(data), content_type='application/json')
...
def build_diff_detail_ajax(request):
    ...
    #sql = "select JobID,UpdateID,PatchFileName,PatchFileMD5,PatchFileSize,FromVersion, Version from diffdetails WHERE JobID = "+str(selectedJobId)+" and "+str(engineTypeWhere)+" order by "+order_column+" "+order_column_dir+" limit "+str(start)+","+str(length)+" "
    ...
    data = []
    for row in data_row:
        data.append({'JobID': str(row[0]), 'UpdateID': config.UpdateType[row[1]], 'PatchFileName': str(row[2]), 'PatchFileMD5': str(row[3]), 'PatchFileSize': flib.getFileSize(int(row[4])), 'FromVersion': str(row[5]), 'Version': str(row[6]), 'FileCount': str(row[7])})
    resp = {"draw": draw, "data": data, "recordsTotal": total_rows, "recordsFiltered": total_rows}
    return HttpResponse(json.dumps(resp), content_type='application/json')

@database_connection_check
def getDiffBuildDetailsDownload(jobId,updateId,updateIDArr,engineType):
    content = '<div class="build-details"><div class="build-basic build-summary shadow-in detail-head "><h5>' + str(config.UpdateTypeOnly[int(updateId)]) + ' <a href="/TableToCsv/?jobId='+str(jobId)+'&engineType='+engineType+'&table=buildDiffDetails&updatetype='+updateId+'""  class="pull-right navbar-inverse shadow-in  extractCSV" ><span class="glyphicon glyphicon-export" ></span></a> <a data-toggle="collapse" data-target="#buildDiffDetails'+str(updateId)+'Table" id="buildDiffDetails'+str(updateId)+'Target" class="floatRight navbar-inverse shadow-in historyBtn">Show Table</a></h5></div>';
...
def build_solo_detail_ajax(request):
    ...
    for engineType in enginemasterData:
        if engineTypeWhere == '':
            engineTypeWhere += '(t1.UpdateID = '+str(engineType[0])+' AND t2.UpdateID = '+str(engineType[0])+')'
        sql = "SELECT t1.*, t2.FileName FROM solodetails t1, deffileinfo t2 where t1.DefID = t2.DefID  and t1.JobID = "+str(selectedJobId)+" AND  ( "+engineTypeWhere+" ) order by "+order_column+" "+order_column_dir+" limit "+str(start)+","+str(length)+""
        data_row = flib.db_query(sql)
        data = []
        for row in data_row:
            data.append({'JobID': str(row[0]), 'UpdateID': config.UpdateType[row[1]], 'FileState': str(row[2]), 'FileMD5': str(row[3]), 'FileSize': flib.getFileSize(int(row[4])), 'DefID': str(format(int(row[5]), '02X')), 'DefSize': flib.getFileSize(int(row[6])), 'FileName': str(row[7])})
        resp = {"draw": draw, "data": data, "recordsTotal":total_rows,"recordsFiltered":total_rows}
    return HttpResponse(json.dumps(resp), content_type='application/json')

def getSoloBuildDetailsDownload(jobId,updateId,updateIDArr,engineType):
    content = ''
    content += '<br/><div class="build-details" id="u_'+str(updateId)+'" ><div class="build-basic build-summary shadow-in detail-head "><h5>'+str(config.UpdateTypeOnly[int(updateId)]) +'  <a href="/TableToCsv/?jobId='+str(jobId)+'&engineType='+engineType+'&table=soloBuildDetails&updatetype='+updateId +'" class="pull-right navbar-inverse shadow-in extractCSV" ><span class="glyphicon glyphicon-export"></span> </a> <a data-toggle="collapse" data-target="#soloBuildDetails'+str(updateId)+'Table" id="soloBuildDetails'+str(updateId)+'Target" class="pull-right navbar-inverse shadow-in historyBtn">Show Table</a></h5></div>';
    ...
    content += '<table class="subtbl">'
    content += '<tr><th class="subHead shadow-in"  rowspan="4">' + str(config.UpdateType[int(updateId)]) + '</th>'
    content += '<tr><th>Version</th><td>'+  format(int(flib.getJobVersion(jobId,updateId)), '02X')  + '</td></tr>'
...
@database_connection_check
def getSoloInfo(jobId,flag,JobStatus,updateIDArr,engineType):
    content = ''
    content += '<h4 data-toggle="collapse" data-target="#soloBuildDetailsDiv" id="solo-detail-head" class="shadow-in">Solo Details <i id="soloDetails" class="soloBuildDetailsDiv glyphicon glyphicon-plus pull-right"></i></h4>'
    ...
    #test = format(int(flib.getJobVersion(jobId, updateIDArr)), '02X')
    #test =  format(int(flib.getJobVersion(jobId,updateId)), '02X')

    for updatetype in updateIDArr:
        content += '<div id="soloSummary">'
        content += getSoloBuildDetailsDownload(jobId, str(updatetype),updateIDArr,engineType)
...
@database_connection_check
def getDiffInfo(jobId,flag,JobStatus,updateIDArr,engineType):
    content = ''
    content += '<h4 data-toggle="collapse" data-target="#buildDiffDetailsDiv" id="diff-detail-head" class="shadow-in">Differential Details <i class="buildDiffDetailsDiv glyphicon glyphicon-plus pull-right" ></i></h4>' \
    ...
    for updatetype in updateIDArr:
        content += '<div id="soloSummary">'
        content += getDiffBuildDetailsDownload(jobId, str(updatetype),updateIDArr,engineType)
...
def getTableToCsv(request):
    ...
    if request.method == 'GET' and 'jobId' in request.GET and 'table' in request.GET and 'updatetype' in request.GET:
        if request.GET['table'] and request.GET['updatetype'] :
            ...
            UpdateTypeOnly = config.UpdateTypeOnly[int(updatetype)]
        if (tableType == 'soloBuildDetails'):
            sql = "SELECT t1.*, t2.FileName FROM solodetails t1, deffileinfo t2 where t1.DefID = t2.DefID and t1.JobID = " +jobId+ " and t1.UpdateID = " +updatetype  + " and t2.UpdateID = " +updatetype  + " order by t1.DefID desc"
            data_row = flib.db_query(sql)
            count = len(data_row)
            response['Content-Disposition'] = 'attachment; filename="'+str(jobId)+'_'+str(engineType)+'_'+str(tableType)+'_'+str(UpdateTypeOnly)+'_('+str(count)+')_records_'+time+'.csv"'
            writer = csv.writer(response)
            writer.writerow( ['Def ID', 'Def Size', 'File State', 'File Size', 'File Name', 'File MD5'])
            for row in data_row:
                writer.writerow([format(int(row[5]), '02X'),flib.getFileSize(int(row[6])),str(row[2]),flib.getFileSize(int(row[4])),str(row[7]), str(row[3])])
            return response
        else:
