    ...
    if type == 'e2':
        ...
        if str(step) == str(1):
            ...
            try:
                ...
                vdb = apiResult['vdb'].strftime("%Y-%m-%d %H:%M:%S")
            ...
            try:
                ...
                if request.method == 'POST':
                    ...
                    if  'Note' in request.POST and request.POST['Note'] != '':
                        ...
                        note = reqType['Note']
                    sql = "INSERT INTO releaseinfo (`JobID`, `Notes`, `VDB`, `UserName`) VALUES ('"+str(jobId)+"','"+str(note)+"','"+str(vdb)+"', '"+str(userName)+"');"
                    try:
...
def build_engine_mgt_ajax(request,type,key=0,action=0):
    ...
    if not type in config.EngineTypeArr:
        return HttpResponse('Invalid request')
    if str(key) != str(0):
        #print('_______',{request.POST['UpdateID']:str(key)},request.user.username,'Action =',str(action))
        flib.markBuildEngine({request.POST['UpdateID']:str(key)},str(action),request.user.username,'')
...
def getUpdateId(request,type,getUpdateID =''):
    updateIdList = queries.EngineType_UpdateID(int(type))
    updateIdDropdown = ''
...
def build_id_details(request,pid):
    ...
    if updateIDArr:
        content += getSoloInfo(request,jobId,flag,JobStatus,updateIDArr,config.EngineType[jobInfo[3]])
        content += getDiffInfo(request,jobId,flag,JobStatus,updateIDArr,config.EngineType[jobInfo[3]])
...
def patch_file_details_ajax(request):
    ...
    resp = {"data": patchFileDetails,'SoloSizeTotal':str(SoloSizeTotal),'DiffSizeTotal':str(DiffSizeTotal)}
    return HttpResponse(json.dumps(resp), content_type='application/json')
