 ...
 (function($) {
	...
	**/
	...
	var parseDate = function (str, opt_date_format) {
		...
		if(opt_date_format != null){
			...
			var df = opt_date_format.replace(/(-|\/)/g, '[-\/]')
				...
				.replace(/(YY|MM|DD|hh|mm|ss)/g, '(\\d{1,2})')
				.replace(/(M|D|h|m|s)/g, '(\\d{1,2})');
			re = new RegExp(df);
			m = re.exec(str);
			if( m != null){
	...
	var draw_date = function($picker, option, date) {
		...
		draw($picker, option, date.getFullYear(), date.getMonth(), date.getDate(), date.getHours(), date.getMinutes(), date.getSeconds());
	};
