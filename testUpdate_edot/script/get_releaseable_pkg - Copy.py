...
def RetrieveToBeReleasePkgListInternal(pkg_list, pkg_list_with_url):
    ...
    #
    sql_query = """select pkg_url, pkg_name from package_info where pkg_id IN
    (select pkg_id from build_info where status_id IN
...
def RetrieveToBeReleasePkgList():
    ...
    iRet = RetrieveToBeReleasePkgListInternal(pkg_list, pkg_list_with_url)
    if 0 != iRet:
    ...
    if 0 == iRet:
        return iRet, pkg_list, pkg_list_with_url
    else:
        return iRet, None, None


def main():
    status, pkg_list1, pkg_list_with_url1 = RetrieveToBeReleasePkgList()
    if status == 0:
        print(pkg_list1, pkg_list_with_url1)
    elif status == 1:
        print(status, 'Error in release package file.')
    else:
        print(status, 'No release package found.')
    sys.exit(0)
